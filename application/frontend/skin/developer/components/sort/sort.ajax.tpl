...
<div class="sort js-search-sort {$sSortClasses}">
	...
	<ul class="nav nav--stacked nav--dropdown dropdown-menu js-search-sort-menu" id="js-dropdown-sort-{$sSortName}">
		{foreach $aSortList as $aSortItem}
			<li class="nav-item sort-item {if $aSortItem@index == 0}active{/if}" data-search-type="{$sSortSearchType}" data-name="sort_by" data-value="{$aSortItem['name']}" data-order="{if $aSortItem['order']}{$aSortItem['order']}{else}desc{/if}">
				<a href="#">
					{$aSortItem['text']}
				</a>
			</li>
