			...
			<li {if $sAction=='profile'}class="active"{/if}>
				...
				{if $sAction=='profile'}
					<ul class="sub-menu" >
						...
						<li {if $aParams[0]=='favourites' and $aParams[1]==''}class="active"{/if}><div><a href="{$oUserProfile->getUserWebPath()}favourites/">{$aLang.user_menu_profile_favourites}</a>{if $iCountTopicFavourite} ({$iCountTopicFavourite}){/if}</div></li>	
						<li {if $aParams[1]=='comments'}class="active"{/if}><div><a href="{$oUserProfile->getUserWebPath()}favourites/comments/">{$aLang.user_menu_profile_favourites_comments}</a>{if $iCountCommentFavourite} ({$iCountCommentFavourite}){/if}</div></li>					
