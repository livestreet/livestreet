...
<form action="{router page='admin'}plugins/" method="post" id="form_plugins_list">
	...
	<input type="submit" name="submit_plugins_del" value="{$aLang.plugins_submit_delete}" onclick="return (jQuery('.form_plugins_checkbox:checked').size()==0)?false:confirm('{$aLang.plugins_delete_confirm}');" />				
</form>
